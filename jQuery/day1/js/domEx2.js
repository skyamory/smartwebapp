...
*/
//btn1 버튼을 클릭하면 inner 클래스가 있는 요소 마지막에 '<br><b>추가 테스트</b><br>'를 추가.
$("#btn1").click(function(){
    $(".container .inner").append("<br><b>추가 테스트</b><br>");
});
//'<br><b>추가 테스트</b><br>'를 container 클래스가 있는 요소 맨 앞에 추가
$("<br><b>추가 테스트</b><br>").appendTo(".container");
//inner 클래스를 포함하는 요소들 중에서 첫번째 요소 맨 앞에 'prepend 테스트 <br>'를 추가.
$(".container .inner").first().prepend("<button>프리펜드</button>");

//append와 prepend는 위치 안에 자식요소로 들어가는데, before와 after는 인접요소로 들어감.
$(".container2 .inner").last().before("<button>B버튼</button>");
$(".container2 .inner").last().after("<button>A버튼</button>");
//사과 css
